module JindaEngine::Concerns::JindaGeneralConcern
  ...
  def logs
    @xmains = Jinda_engine::Xmain.all.desc(:created_at).page(params[:page]).per(10)
  end

  def error_logs
    @xmains = Jinda_engine::Xmain.in(status:['E']).desc(:created_at).page(params[:page]).per(10)
  end

  def notice_logs
    @notices= Jinda_engine::Notice.desc(:created_at).page(params[:page]).per(10)
  end

  def pending
    @title= "Pending Tasks"
    @xmains = Jinda_engine::Xmain.in(status:['R','I']).asc(:created_at)
  end

  def cancel
    Jinda_engine::Xmain.find(params[:id]).update_attributes :status=>'X'
    if params[:return]
  ...
  def document
    doc = Jinda_engine::Doc.find params[:id]
    if doc.cloudinary
      require 'net/http'
      require "uri"
  ...
  def doc
    ...
    @print= "<div align='right'><img src='/assets/printer.png'/> <a href='/Jinda_engine/doc_print' target='_blank'/>Print</a></div>"
  ...
  def status
    @xmain= Jinda_engine::Xmain.where(:xid=>params[:xid]).first
