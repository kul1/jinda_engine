module JindaEngine::Concerns::JindaRunConcern
  ...
  def run_form
    ...
    if authorize?
      if ['F', 'X'].include? @xmain.status
        ...
        ###############################################################################################
        # Run View Form f created template by jinda_engine rake follow freemind mm file
        ###############################################################################################
        if service
  ...
  def run_list
    ...
    if service && !@runseq.code.blank? 
      f= "app/views/#{service.module.code}/#{service.code}/#{@runseq.code}.html.erb"
      @ui= File.read(f)
      if jinda_engine::Doc.where(:runseq_id=>@runseq.id).exists?
        @doc= jinda_engine::Doc.where(:runseq_id=>@runseq.id).first
        ...
        @doc= jinda_engine::Doc.create :name=> @runseq.name,
      ...
      eval "@xvars[@runseq.code] = url_for(:controller=>'jinda_engine', :action=>'document', :id=>@doc.id)"
    else
      flash[:notice]= "Error: Can not find the view file for this controller"
      ma_log "Error: Can not find the view file for this controller"
      redirect_to_root
  ...
  def run_mail
    ...
    jinda_engineMailer.gmail(@doc.data_text, recipients, subject).deliver unless DONT_SEND_MAIL
    end_action
  end
